...
export default {
  ...
  patchEditor(editor) {
    ...
    element.wheelListener = (e) => {
      let scrollSensitivity = editor.getScrollSensitivity()/100 ; let isChanged
      if (e.shiftKey) {
        isChanged = editor.scrollAnimation(-e.wheelDeltaY*scrollSensitivity, 0, 0, false)
      } else {
        isChanged = editor.scrollAnimation(0, -e.wheelDeltaY*scrollSensitivity, 0, false)
      }
