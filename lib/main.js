  ...
  patchEditor(editor) {
    ...
    element.scrollAnimation = (props) => {
      ...
      if (props.lastCursor) {
        ...
        if (!props.center) {
          if (pendingScrollY>0) {
            pendingScrollY = Math.max(0, pendingScrollY-scrollMargin)
          } else {
      ...
      // fix rest step
      if (pendingScrollX) { pendingScrollX += (divider-1)*Math.sign(pendingScrollX) }
      if (pendingScrollY) { pendingScrollY += (divider-1)*Math.sign(pendingScrollY) }
      scrollAnimation()
    }
