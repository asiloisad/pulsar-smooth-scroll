    ...
    this.disposables.add(
      ...
      atom.commands.add('atom-text-editor:not([mini])', {
        ...
        'smooth-scroll:scroll-up':
          () => this.scrollUpOne(),
        'smooth-scroll:scroll-up-all':
          () => this.scrollUpAll(),
        'smooth-scroll:scroll-down':
          () => this.scrollDownOne(),
        'smooth-scroll:scroll-down-all':
          () => this.scrollDownAll(),
  ...
  patchEditor(editor) {
    ...
    element.scrollAnimation = (props) => {
      ...
      if (props.lastCursor) {
        ...
        if (props.center) {
          const scrollMargin = (component.getScrollContainerClientHeight()- component.getVerticalAutoscrollMargin()+editor.getLineHeightInPixels())/2
          pendingScrollY = pixelPosition.top-component.scrollTop-scrollMargin
        } else {
    ...
    element.smoothScrollListener = (e) => {
      ...
      if (this.ctrlMode && e.ctrlKey) {
        for (let pane of atom.workspace.getCenter().getPanes()) {
          ...
          item.element.scrollAnimation({ valueX:valueX*sv, valueY:valueY*sv, divider:'wheel' })
        }
      } else {
        let sv = editor.getScrollSensitivity()
        element.scrollAnimation({ valueX:valueX*sv, valueY:valueY*sv, divider:'wheel' })
      }
    }
