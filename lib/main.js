...
module.exports = {
  ...
  patchEditor(editor) {
    ...
    element.smoothScrollListener = (e) => {
      // Scroll chaining: if the editor is already at the boundary in the scroll
      // direction, let the event propagate so a parent container can scroll instead.
      // When horizontalWithShift is active the vertical wheel delta is redirected
      // to the horizontal axis, so fold it into deltaX before the boundary check.
      ...
      const canScrollY =
        deltaY < 0
          ...
          : deltaY > 0
            ? component.scrollTop < component.getMaxScrollTop()
            : false;
      const canScrollX =
        deltaX < 0
          ? component.scrollLeft > 0
          : deltaX > 0
            ? component.scrollLeft < component.getMaxScrollLeft()
            : false;
  ...
  calculateDividerStep(pending, smoothness) {
    return Math.trunc(pending / smoothness);
  },
